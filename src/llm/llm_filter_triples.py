...
def llm_filter_triples_main(dataset_name=None,llm=None,resume_path=None):
    return asyncio.run(llm_filter_triples(dataset_name,llm,resume_path))


async def llm_filter_triples(dataset_name=None,llm=None,resume_path=None):
    # 加载路径
    ...
    tasks = [
        process_single_query(llm_chat, question_id, each_filter_triples_prompt, mode)
